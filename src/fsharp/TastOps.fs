...
type ValMap<'T>(imap: StampMap<'T>) = 
    ...
    member m.IsEmpty = imap.Count = 0
    static member OfList (vs: (Val*'T) list) = ValMap<'T>(StampMap.OfListMapped (fun (k:Val,v) -> KeyValuePair<_,_>(k.Stamp,v)) vs)
...
type ValMultiMap<'T>(contents: StampMap<'T list>) =
    ...
    static member Empty = ValMultiMap<'T>(StampMap.Empty)
