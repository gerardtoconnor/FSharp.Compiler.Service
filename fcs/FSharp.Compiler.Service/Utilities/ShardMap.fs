...
// Reuse standard MapTree with slight modifications for count mutation
...
type ShardMap<'K,'V  when 'K : equality and 'K : comparison >(icount:int, newBucket:Shard<'K,'V> []) =
    ...
    let addMany(kvl: #seq<('K * 'V)>)=

        // let kh = k.GetHashCode()
        // let bi = util.bucketIndex(kh,bucketBitMask)
        // let si = util.shardIndex kh
        let copied = Array.zeroCreate(bucket.Length) // this will track if new shards created to avoid creating more then once when not needed
        for (k,v) in kvl do
            let idx = getIndexes(k,bucketBitMask)
            let shrd = 
                if copied.[idx.bi] then
                    bucket.[idx.bi] // new copy aready created so re-use in this scope safely
                else
                    copied.[idx.bi] <- true
                    let shrd = newShardCopy bucket.[idx.bi] // a copy of initial/original
                    lock bucket.SyncRoot (fun () -> bucket.[idx.bi] <- shrd)
                    shrd
